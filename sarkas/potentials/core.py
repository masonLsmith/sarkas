...
class Potential:
    ...
    def setup(self, params) -> None:
        ...
        if not self.type.lower() == "fmm":
            ...
            if self.rc > params.box_lengths.min() / 2.0:
                print(
                    "\nWARNING: The cut-off radius is > L/2. L/2 = ",
                    params.box_lengths.min() / 2,
                    "will be used as rc",
                )
                self.rc = params.box_lengths.min() / 2.0
                self.linked_list_on = False  # linked list off
        ...
        if self.type.lower() == "qsp":
            ...
            params.ne = (
                params.species_charges.transpose()
                @ params.species_concentrations
                * params.total_num_density
                / params.qe
            )
        ...
        if hasattr(self, "kappa"):
            if self.electron_temperature != params.total_ion_temperature:
                ...
                print(
                    "You have defined both kappa and the electron_temperature. kappa's value will be used."
                )
                print(
                    "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!\n"
                )
    ...
    def calc_electron_properties(params):
        ...
        lambda_TF_sq /= (
            spin_degeneracy / np.sqrt(np.pi) * fdint_fdk_vec(k=-0.5, phi=params.eta_e)
        )
        ...
        params.electron_degeneracy_parameter = (
            params.kB * params.electron_temperature / params.fermi_energy
        )
        params.relativistic_parameter = (
            params.hbar * params.kF / (params.me * params.c0)
        )
        ...
        params.electron_coupling = params.qe ** 2 / (
            ...
            * params.fermi_energy
            * params.ae_ws
            * np.sqrt(params.electron_degeneracy_parameter ** 2)
        )

        # Warm Dense Matter Parameter, Eq.3 in Murillo Phys Rev E 81 036403 (2010)
        params.wdm_parameter = 2.0 / (
            params.electron_degeneracy_parameter
            + 1.0 / params.electron_degeneracy_parameter
        )
        params.wdm_parameter *= 2.0 / (
            params.electron_coupling + 1.0 / params.electron_coupling
        )
        ...
        if params.magnetized:
            if params.units == "cgs":
                params.electron_cyclotron_frequency = (
                    params.qe
                    * np.linalg.norm(params.magnetic_field)
                    / params.c0
                    / params.me
                )
            else:
                params.electron_cyclotron_frequency = (
                    params.qe * np.linalg.norm(params.magnetic_field) / params.me
                )

            params.electron_magnetic_energy = (
                params.hbar * params.electron_cyclotron_frequency
            )
            tan_arg = 0.5 * params.hbar * params.electron_cyclotron_frequency * beta_e

            # Perpendicular correction
            params.horing_perp_correction = (
                params.electron_plasma_frequency / params.electron_cyclotron_frequency
            ) ** 2
            params.horing_perp_correction *= 1.0 - tan_arg / np.tanh(tan_arg)
            params.horing_perp_correction += 1

            # Parallel correction
            params.horing_par_correction = (
                1
                - (params.hbar * beta_e * params.electron_plasma_frequency) ** 2 / 12.0
            )

            # Quantum Anisotropy Parameter
            params.horing_delta = params.horing_perp_correction - 1
            params.horing_delta += (
                params.hbar * beta_e * params.electron_cyclotron_frequency
            ) ** 2 / 12
    ...
    def update_linked_list(self, ptcls):
        ...
        """
        ptcls.potential_energy, ptcls.acc = force_pp.update(
            ptcls.pos,
            ptcls.id,
            ptcls.masses,
            self.box_lengths,
        ...
        if not (self.type == "LJ"):
            ...
            ptcls.potential_energy += (
                ...
                * np.pi
                * np.sum(dipole ** 2)
                / (3.0 * self.box_volume * self.fourpie0)
            )
    ...
    def update_pm(self, ptcls):
        ...
        U_long += (
            -np.pi
            * self.total_net_charge ** 2.0
            / (2.0 * self.box_volume * self.pppm_alpha_ewald ** 2)
        )
    ...
    def pppm_setup(self, params):
        ...
        params.pppm_pm_err *= (
            np.sqrt(params.total_num_ptcls) * params.a_ws ** 2 * params.fourpie0
        )
