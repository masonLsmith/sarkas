...
def update_params(potential, params):
    ...
    for i, q1 in enumerate(params.species_charges):
        for j, q2 in enumerate(params.species_charges):
            ...
            potential.matrix[1, i, j] = 0.5 * (
                params.species_lj_sigmas[i] + params.species_lj_sigmas[j]
            )
    ...
    params.force_error *= (
        np.sqrt(params.total_num_ptcls / params.pbox_volume) * params.a_ws ** 2
    )
...
def lj_force(r, pot_matrix_ij):
    ...
    force = (
        epsilon
        * (pot_matrix_ij[2] * s_over_r_high - pot_matrix_ij[3] * s_over_r_low)
        / r
    )
