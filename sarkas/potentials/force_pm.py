...
def force_optimized_green_function(box_lengths, mesh_sizes, aliases, p, constants):
    ...
    nz_mid = (
        mesh_sizes[2] / 2 if np.mod(mesh_sizes[2], 2) == 0 else (mesh_sizes[2] - 1) / 2
    )
    ny_mid = (
        mesh_sizes[1] / 2 if np.mod(mesh_sizes[1], 2) == 0 else (mesh_sizes[1] - 1) / 2
    )
    nx_mid = (
        mesh_sizes[0] / 2 if np.mod(mesh_sizes[0], 2) == 0 else (mesh_sizes[0] - 1) / 2
    )
            ...
            for nx in range(mesh_sizes[0]):
                ...
                if k_sq != 0.0:
                    ...
                    for mz in range(-aliases[2], aliases[2] + 1):
                        ...
                        U_kz_M = (
                            np.sin(0.5 * kz_M * h_array[2]) / (0.5 * kz_M * h_array[2])
                            if kz_M != 0.0
                            else 1.0
                        )
                        ...
                        for my in range(-aliases[1], aliases[1] + 1):
                            ky_M = (
                                two_pi * (ny_sh + my * mesh_sizes[1]) / box_lengths[1]
                            )
                            U_ky_M = (
                                np.sin(0.5 * ky_M * h_array[1])
                                / (0.5 * ky_M * h_array[1])
                                if ky_M != 0.0
                                else 1.0
                            )
                            ...
                            for mx in range(-aliases[0], aliases[0] + 1):
                                kx_M = (
                                    two_pi
                                    * (nx_sh + mx * mesh_sizes[0])
                                    / box_lengths[0]
                                )
                                U_kx_M = (
                                    np.sin(0.5 * kx_M * h_array[0])
                                    / (0.5 * kx_M * h_array[0])
                                    if kx_M != 0.0
                                    else 1.0
                                )
                                ...
                                G_k_M = (
                                    four_pi
                                    * np.exp(-0.25 * (kappa_sq + k_M_sq) / Gew_sq)
                                    / (kappa_sq + k_M_sq)
                                )
                    ...
                    Gk_hat = (
                        four_pi
                        * np.exp(-0.25 * (kappa_sq + k_sq) / Gew_sq)
                        / (kappa_sq + k_sq)
                    )

                    # eq.(28) of Ref.[Dharuman2017]_
                    PM_err += Gk_hat * Gk_hat * k_sq - U_G_k ** 2 / (
                        (U_k_sq ** 2) * k_sq
                    )
    ...
    for ipart in range(N):
        ...
        for g in range(cao):
            ...
            for i in range(cao):
                ...
                r_i = (
                    iyn + mesh_sz[1] * (iyn < 0) - mesh_sz[1] * (iyn > (mesh_sz[1] - 1))
                )
        ...
        for g in range(cao):
            ...
            for i in range(cao):
                ...
                for j in range(cao):
                    ...
                    E_x_p[ipart] += (
                        q_over_m * E_x_r[r_g, r_i, r_j] * wz[g] * wy[i] * wx[j]
                    )
                    E_y_p[ipart] += (
                        q_over_m * E_y_r[r_g, r_i, r_j] * wz[g] * wy[i] * wx[j]
                    )
                    E_z_p[ipart] += (
                        q_over_m * E_z_r[r_g, r_i, r_j] * wz[g] * wy[i] * wx[j]
                    )
...
def update(pos, charges, masses, mesh_sizes, box_lengths, G_k, kx_v, ky_v, kz_v, cao):
    ...
    acc_f = calc_acc_pm(
        E_x_r, E_y_r, E_z_r, pos, charges, N, cao, masses, mesh_sizes, mesh_spacings
    )
